...
public class SimSettings {
	...
	private int NUM_OF_PLACE_TYPES; // 平台类型的数量
	...
	private int NUM_OF_HOST_ON_CLOUD_DATACENTER; //云数据中心主机数量
	private int NUM_OF_VM_ON_CLOUD_HOST; // 云主机中虚拟机的数量
	private int CORE_FOR_CLOUD_VM; //
	private int MIPS_FOR_CLOUD_VM; //MIPS  计算机的执行速度
	private int RAM_FOR_CLOUD_VM; //MB 内存
	...
	private String[] SIMULATION_SCENARIOS; //模拟场景
	private String[] ORCHESTRATOR_POLICIES; //调度策略
	...
	// [0] usage percentage (%)
	// [1] prob. of selecting cloud (%)  选择云的概率
	// [2] poisson mean (sec) 平均泊松
	// [3] active period (sec)  活跃期
	// [4] idle period (sec)  闲置期
	// [5] avg data upload (KB)
	// [6] avg data download (KB)
	// [7] avg task length (MI) 平均任务长度
	// [8] required # of cores
	// [9] vm utilization on edge (%)  边缘虚拟机利用率
	// [10] vm utilization on cloud (%)
	// [11] vm utilization on mobile (%) 移动设备上的虚拟机利用率
	// [12] delay sensitivity [0-1]  延迟灵敏度
	 ...
	 */
	// 阵列内的移动性特征结果包括平均等待时间（分钟）或每个地点类型
	...
	private void parseApplicationsXML(String filePath)
	{
		...
		try {
			...
			String mandatoryAttributes[] = {
					...
					"idle_period", //idle period (sec)  空闲时间
		...
		try {	
			...
			for (int i = 0; i < datacenterList.getLength(); i++) {
				...
				for (int j = 0; j < hostList.getLength(); j++) {
					NUM_OF_EDGE_HOSTS++; // 数据中心上运行的边缘主机数
