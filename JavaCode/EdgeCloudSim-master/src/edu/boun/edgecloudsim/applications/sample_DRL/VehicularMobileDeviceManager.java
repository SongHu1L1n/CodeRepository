...
//import edu.boun.edgecloudsim.applications.sample_app5.VehicularCpuUtilizationModel;
...
public class VehicularMobileDeviceManager extends MobileDeviceManager {
	...
	public UtilizationModel getCpuUtilizationModel() {
		return new VehicularCpuUtilizationModel();// CPU利用率模型
	}
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		{
			((VehicularNetworkModel)networkModel).updateMM1QueueModel();
		...
		{
			Task task = (Task) ev.getData();
			submitTaskToVm(task, VM_TYPES.CLOUD_VM);
			break;
		}
		case REQUEST_RECEIVED_BY_EDGE_DEVICE:
		{
			Task task = (Task) ev.getData();			
			submitTaskToVm(task, VM_TYPES.EDGE_VM);
			break;
		}
		case REQUEST_RECEIVED_BY_MOBILE_DEVICE:
		{
			Task task = (Task) ev.getData();			
			submitTaskToVm(task, VM_TYPES.MOBILE_VM);
			break;
		}
